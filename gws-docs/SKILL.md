...
- Requires the [`gws` CLI](https://github.com/googleworkspace/cli) on `$PATH`.
- Authentication:
  - Run `gws auth login` to authenticate with your Google account, or
  - Set `export GOOGLE_APPLICATION_CREDENTIALS=/path/to/key.json` for service account access.
